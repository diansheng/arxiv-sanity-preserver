...
def get_latest_or_loop(q, logger):
  results = None
  while results is None:
    try:
      results = api.GetSearch(raw_query="q=%s&result_type=recent&count=100" % (q, ))
    except Exception as e:
      logger.error('There was a problem with Twitter API:')
      logger.error(str(e))
...
if __name__ == "__main__":
  ...
  parser = argparse.ArgumentParser(description='Twitter daemon for arXiv papers')
  ...
  logger = setup_logger('twitter_daemon', log_level=log_level, log_file=args.log_file)
  
  logger.info("Starting Twitter daemon")
  ...
  logger.info(f"mongodb tweets collection size: {tweets.count()}")
  ...
  if os.path.isfile(Config.banned_path):
    ...
    logger.info(f"Banning users: {list(banned.keys())}")
  ...
  # fetch all database arxiv pids that we know about (and handle an upadte of the db file)
    if last_db_load is None or os.stat(Config.db_path).st_mtime > last_db_load:
      ...
      logger.info(f"(re-) loading the paper database {Config.db_path}")
      db = pickle.load(open(Config.db_path, 'rb'))

  # fetch the latest mentioning arxiv.org
    results = get_latest_or_loop('arxiv.org', logger)
  to_insert = []
  for r in results:
  ...
  logger.info(f"Processed {len(to_insert)}/{len(results)} new tweets. Currently maintaining total {tweets.count()}")
  ...
  for days in [1,7,30]:
    ...
    logger.info(f"Top votes for {days} days: {votes[:min(len(votes), 10)]}")
