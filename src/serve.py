...
from src.utils import safe_pickle_dump, strip_version, isvalidid, Config, setup_logger
...
if __name__ == "__main__":
  ...
  parser.add_argument('--log-file', dest='log_file', type=str, help='path to log file')
  args = parser.parse_args()
  
  # Setup logging
  log_level = logging.DEBUG if args.verbose else logging.INFO
  logger = setup_logger('serve', log_level=log_level, log_file=args.log_file)
  
  logger.info(f"Starting with arguments: {args}")

  if not os.path.isfile(Config.database_path):
    logger.warning('did not find as.db, trying to create an empty database from schema.sql...')
    logger.warning('this needs sqlite3 to be installed!')
    os.system('sqlite3 as.db < schema.sql')

  logger.info(f"loading the paper database {Config.db_serve_path}")
  db = pickle.load(open(Config.db_serve_path, 'rb'))
  
  logger.info(f"loading tfidf_meta {Config.meta_path}")
  meta = pickle.load(open(Config.meta_path, "rb"))
  vocab = meta['vocab']
  idf = meta['idf']

  logger.info(f"loading paper similarities {Config.sim_path}")
  sim_dict = pickle.load(open(Config.sim_path, "rb"))

  logger.info(f"loading user recommendations {Config.user_sim_path}")
  user_sim = {}
  if os.path.isfile(Config.user_sim_path):
    user_sim = pickle.load(open(Config.user_sim_path, 'rb'))
  
  logger.info(f"loading serve cache... {Config.serve_cache_path}")
  ...
  logger.info(f"mongodb tweets_top1 collection size: {tweets_top1.count()}")
  logger.info(f"mongodb tweets_top7 collection size: {tweets_top7.count()}")
  logger.info(f"mongodb tweets_top30 collection size: {tweets_top30.count()}")
  logger.info(f"mongodb comments collection size: {comments.count()}")
  logger.info(f"mongodb tags collection size: {tags_collection.count()}")
  logger.info(f"mongodb goaway collection size: {goaway_collection.count()}")
  logger.info(f"mongodb follow collection size: {follow_collection.count()}")
